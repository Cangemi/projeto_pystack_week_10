...
def home(request):
    if request.method == 'GET':
        ...
        proximas_consultas = Consulta.objects.filter(paciente=request.user).filter(data_aberta__data__gte= hoje + timedelta(days=1))
        ...
        return render(request, 'home.html', {'medicos': medicos, 'especialidades': especialidades,'proximas_consultas': proximas_consultas,'hoje':hoje,'is_medico':is_medico(request.user)})
...
def minhas_consultas(request):
    # Realizar os filtros
    if request.method == "GET":
        minhas_consultas = Consulta.objects.filter(paciente=request.user).filter(data_aberta__data__gte=datetime.now() - timedelta(days=1)).order_by('data_aberta__data')
